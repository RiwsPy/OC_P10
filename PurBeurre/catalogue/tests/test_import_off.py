...
from catalogue.management.commands.enums import HEADERS, PAYLOAD, CATEGORIES, URL_SEARCH, URL_OFF
...
class Test_ImportAPIData(TestCase):
    ...
    def test_request_ok_call_api(self, mock_request):
        ...
        with open(os.path.join(Path(__file__).resolve().parent, "db_product_mock.json")) as file:
            file = load(file)
